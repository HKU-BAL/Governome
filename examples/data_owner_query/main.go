...
func Queryuser_Trivium(Parameter tfhe.ParametersLiteral[uint32], user_name, rsid string, Readsymbol bool, Verifysymbol bool) {
	...
	if Readsymbol {
		segID := auxiliary.SegmentID(user_name, auxiliary.RsID_s2i(rsid), auxiliary.Seg_num)
		segkey1 = snarks.ReadSegKey(user_name, 1, segID, 1, params)
		segkey2 = snarks.ReadSegKey(user_name, 2, segID, 1, params)
	} else {
		Indiv := make([]string, 1)
		Indiv[0] = user_name
	...
	if Verifysymbol {
		...
		segID := auxiliary.SegmentID(user_name, auxiliary.RsID_s2i(rsid), auxiliary.Seg_num)
		keyhash1, keyhash2 := trivium.ReadKeyhash(user_name, 1)
		ccs := snarks.GenorReadR1CS(circuit)
		_, VerifyKey := snarks.GenorReadSetup(ccs)
		for i := 1; i < 3; i++ {
			proof := snarks.ReadProof(user_name, i, segID, 1)
	...
	gt_ct := trivium.Userquery(user_name, auxiliary.RsID_s2i(rsid), segkey1, segkey2, 1, eval)
	for i := 0; i < 4; i++ {
